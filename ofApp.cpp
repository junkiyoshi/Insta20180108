...
void ofApp::draw() {
	...
	/*
	for (int y = 0; y < this->depth.rows; y += 16) {
		for (int x = 0; x < this->depth.cols; x += 16) {
			int z = depth.at<unsigned short>(y, x);
			if (z != 0) {
				ofSetColor(ofColor(0, 255 * ofNoise(x, y * 0.025 - ofGetFrameNum() * 0.05), 0));

				ofPushMatrix();
				ofTranslate(0, 0, z - 1024);
				this->font.drawString(moji.substr((int)z % 26, 1), x, y);
				ofPopMatrix();
			}
		}
	}
